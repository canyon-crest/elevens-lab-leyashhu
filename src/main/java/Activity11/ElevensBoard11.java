...
public class ElevensBoard11 extends Board11 {
		...
		for (int sk1 = 0; sk1 < selectedCards.size(); sk1++) {
			...
			for (int sk2 = sk1 + 1; sk2 < selectedCards.size(); sk2++) {
				...
				if (cardAt(k1).pointValue() + cardAt(k2).pointValue() == 11) {
					result.add(k1);
					result.add(k2);
					return result; // return first valid pair found
				}
			}
		}
		return result; // empty list if none found
	}
	 ...
	 * @return a list of the indexes of a JQK, if a JQK was found; an empty list, if
	 *         a JQK was not found.
	 */
	...
	public boolean playIfPossible() {
		...
		// If no pair sum 11, try to play a JQK triplet
	...
	private boolean playPairSum11IfPossible() {
		...
		if (pair.size() == 2) {
			replaceSelectedCards(pair); // method from Board11
			return true;
		}
		return false;
	}
	...
	private boolean playJQKIfPossible() {
		...
		if (jqk.size() == 3) {
			replaceSelectedCards(jqk); // method from Board11
			return true;
		}
		return false;
	}
